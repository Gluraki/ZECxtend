...
def test_get_attempts_export_challenge_not_found(db, seeded_attempts, mock_requests):
    _, mock_export = mock_requests
    mock_export.get.side_effect = lambda url, *a, **kw: _make_response(url, challenge_status=404)
    with pytest.raises(EntityDoesNotExistError, match="Challenge not found"):
        crud.get_attempts_export(db=db, challenge_id=1)


def test_get_attempts_export_challenge_service_error(db, seeded_attempts, mock_requests):
    _, mock_export = mock_requests
    mock_export.get.side_effect = lambda url, *a, **kw: _make_response(url, challenge_status=500)
    with pytest.raises(ServiceError, match="Failed to fetch challenge"):
        crud.get_attempts_export(db=db, challenge_id=1)


def test_get_attempts_export_teams_not_found(db, seeded_attempts, mock_requests):
    _, mock_export = mock_requests
    mock_export.get.side_effect = lambda url, *a, **kw: _make_response(url, teams_status=404)
    with pytest.raises(EntityDoesNotExistError, match="Teams not found"):
        crud.get_attempts_export(db=db, challenge_id=1)


def test_get_attempts_export_teams_unauthorized(db, seeded_attempts, mock_requests):
    _, mock_export = mock_requests
    mock_export.get.side_effect = lambda url, *a, **kw: _make_response(url, teams_status=401)
    with pytest.raises(AuthenticationFailed):
        crud.get_attempts_export(db=db, challenge_id=1)


def test_get_attempts_export_teams_forbidden(db, seeded_attempts, mock_requests):
    _, mock_export = mock_requests
    mock_export.get.side_effect = lambda url, *a, **kw: _make_response(url, teams_status=403)
    with pytest.raises(AuthenticationFailed):
        crud.get_attempts_export(db=db, challenge_id=1)


def test_get_attempts_export_teams_service_error(db, seeded_attempts, mock_requests):
    _, mock_export = mock_requests
    mock_export.get.side_effect = lambda url, *a, **kw: _make_response(url, teams_status=500)
    with pytest.raises(ServiceError, match="Failed to fetch teams"):
        crud.get_attempts_export(db=db, challenge_id=1)


def test_get_attempts_export_drivers_not_found(db, seeded_attempts, mock_requests):
    _, mock_export = mock_requests
    mock_export.get.side_effect = lambda url, *a, **kw: _make_response(url, drivers_status=404)
    with pytest.raises(EntityDoesNotExistError, match="Drivers not found"):
        crud.get_attempts_export(db=db, challenge_id=1)


def test_get_attempts_export_drivers_service_error(db, seeded_attempts, mock_requests):
    _, mock_export = mock_requests
    mock_export.get.side_effect = lambda url, *a, **kw: _make_response(url, drivers_status=500)
    with pytest.raises(ServiceError, match="Failed to fetch drivers"):
        crud.get_attempts_export(db=db, challenge_id=1)