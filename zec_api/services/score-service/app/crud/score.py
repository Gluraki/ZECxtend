...
def calculate_f_pm(db_attempt):
    ...
    driver = requests.get(f"{TEAM_URL}/api/drivers/{fastest_attempt['driver_id']}").json()
...
def handle_acceleration(db_attempt):
    ...
    score = f_points * (t_min_seconds / t_team_seconds) + f_pm / 20
    return score
...
def create_score(*, db: SessionDep, score: ScoreCreate):
    ...
    recalculate_acceleration_scores(db=db, challenge_id=db_attempt["challenge_id"])
    db_score = Score(
...
def recalculate_acceleration_scores(*, db: SessionDep, challenge_id: int):
    challenge_response = requests.get(f"{CHALLENGE_URL}/api/challenges/{challenge_id}")
    challenge = challenge_response.json()
    if challenge["name"] != "Acceleration":
        return False
    fastest_attempt_id = None
    fastest_attempt = requests.get(f"{ATTEMPT_URL}/api/attempts/fast/", params={"challenge_id": challenge_id}).json()
    if fastest_attempt["id"] != fastest_attempt_id:
        fastest_attempt_id = fastest_attempt["id"]
        attempts_response = requests.get(f"{ATTEMPT_URL}/api/attempts/", params={"challenge_id": challenge_id})
        attempts = attempts_response.json()
        for attempt in attempts:
            existing_score = db.query(Score).filter(Score.attempt_id == attempt["id"]).first()
            if existing_score:
                score_value = handle_acceleration(attempt)
